...
#ifndef __BPF_CUBIC_H__
#define __BPF_CUBIC_H__
...
static bool before(__u32 seq1, __u32 seq2) { return (__s32)(seq1 - seq2) < 0; }
...
static const __u64 cube_factor =
    (__u64)(1ull << (10 + 3 * BICTCP_HZ)) / (bic_scale * 10);
...
void bpf_cubic_init(struct sock *sk) {
  struct bpf_bictcp *ca = inet_csk_ca(sk);
...
static void bpf_cubic_cong_avoid(struct sock *sk, __u32 ack, __u32 acked) {
  struct tcp_sock *tp = tcp_sk(sk);
  struct bpf_bictcp *ca = inet_csk_ca(sk);
...
static u32 bpf_cubic_recalc_ssthresh(struct sock *sk) {
...
static void bpf_cubic_state(struct sock *sk, __u8 new_state) {
...
static void hystart_update(struct sock *sk, __u32 delay) {
  ...
  if (hystart_detect & HYSTART_DELAY) {
    ...
    if (ca->sample_cnt < HYSTART_MIN_SAMPLES) {
      ...
      if (ca->curr_rtt >
          ca->delay_min + HYSTART_DELAY_THRESH(ca->delay_min >> 3)) {
        ca->found = 1;
        tp->snd_ssthresh = tp->snd_cwnd;
      }
    }
...
static void bpf_cubic_acked(struct sock *sk, const struct ack_sample *sample) {
  const struct tcp_sock *tp = tcp_sk(sk);
  struct bpf_bictcp *ca = inet_csk_ca(sk);
  __u32 delay;
...
u32 bpf_cubic_undo_cwnd(struct sock *sk) { return tcp_reno_undo_cwnd(sk); }
